...
> ⚠️ **開発中です。このプロジェクトはまだ実用段階ではありません。**

Macroquad ベースの、グラフィックス・スプライトリソース管理エンジン。256色パレットモードとフルカラー（RGBA）の両方に対応し、実行中にスプライトを動的に編集できます。

## 📋 プロジェクトステータス

**このプロジェクトはアクティブな開発段階にあります。**

- ✅ コア機能（リソース管理、シリアライゼーション）は実装済み
- ✅ WASM デモ（Cubeboy 3D、Lineboy）はテスト可能
- 🚧 API は不安定で、変更される可能性があります
- 🚧 ドキュメントと例は未完成です
- ❌ **本番環境での使用は推奨されません**

**詳細は [GitHub Issues](https://github.com/oosawak/nQuad/issues) をご確認ください。**
