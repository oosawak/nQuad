...
const FRAMEBUFFER_SIZE: usize = 128 * 128;
static mut FRAMEBUFFER: [u32; FRAMEBUFFER_SIZE] = [0; FRAMEBUFFER_SIZE];
...
impl Player {
    ...
    fn rand_f32() -> f32 {
        ...
        let seed = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos() as u32;
        ((seed ^ (seed << 13) ^ (seed >> 17) ^ (seed << 5)) % 10000) as f32 / 10000.0
    }
...
impl Game {
    ...
    fn render(&self) {
        ...
        // Test: Draw a blue rectangle at top-left
        draw_rect(0, 0, 16, 16, 0x0000FF);
...
fn set_pixel(x: u32, y: u32, color: u32) {
    if x < W && y < H {
        unsafe {
            FRAMEBUFFER[(y * W + x) as usize] = color;
        }
    }
}
